...
export async function signUpMember(req, res) {
  try {
    const { email, password, name, website, organizationType, industry } = req.body;
    ...
    transporter.sendMail(mailOptions, function(error, info){
      if (error) {
        ...
        res.status(201).send({ message: 'Verification email sent', member: addedMember });
      }
    });
  } 
  catch (error) {
...
export async function verifyMember(req, res) {
  ...
  try {
    ...
    if (!member) {
      return res.status(404).send({ message: 'No member found with this token'});
    }
